...
def connect_readwrite():
    try:
        ...
        print 'Database connect error, exit.'
        traceback.print_exc()
...
def read_queue(vm_type, flag):
    res = []
    """
    LIMIT的值与系统支配的虚拟机数量有关，这里的100是临时取的值，
    实际中要根据虚拟机数量来设定。
    """
    LIMIT = 100
    collection = get_queue(vm_type)
...
def encode_str(str_file):
    ...
    try:
        ...
        print 'fail to convert gb2312 to utf8'
        cont = ""
    return cont
...
def log2json(logfile):
    ...
    #print 'file size: ', os.path.getsize(logfile)
...
def read_log(hashvalue):
    ...
    if res != None:
        ...
        print 'if we do not have this hashvalue, you will see this.'
        actionlist = ''
        pcap = ''
        static_info = ''
        ss = ''
